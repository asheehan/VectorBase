...
function fixMungedFilenames() {
	...
	while($record = $results->fetchAssoc()) {
		...
		if($node !== FALSE) {
			if(preg_match('/^(.+)_\.(.+)/', $node->field_file['und'][0]['filename'], $m)) {
				$fixTitle = $node->title === $node->field_file['und'][0]['filename'];
				$newname = "{$m[1]}.{$m[2]}";
				if($newFile = file_move(file_load($node->field_file['und'][0]['fid']), dirname($node->field_file['und'][0]['uri']) . "/$newname", FILE_EXISTS_REPLACE)) {
					$status .= "<li>Changing node titled \"{$node->title}\"<ul><li>{$node->field_file['und'][0]['filename']}&nbsp;(original)</li><li><b>$newname</b>&nbsp;(new)</li>";
					$status .= $fixTitle ? '<li>Change title? <span style="color:green">YES</span></li></ul>' : '<li>Change title? <span style="color:red">NO</span></li></ul>';
					if($fixTitle) {
						$node->title = $newname;
						node_save($node);
					}
					...
					$status .= "<li><span style=\"color:yellow\">There was a prolem fixing node titled \"{$node->title}\"</span></li>";
				}
			}
		}
	}
